<template>
  <div>
    ...
    <div class="container text-white">
      <div class="py-5">
        <h2>Apply request for you account</h2>
        <p class="mb-0 text-muted">(Professional artist)</p>
      </div>
            ...
            <div class="row g-3">
              ...
              <div class="my-0">
                ...
                <p class="my-0 ps-2 text-start text-warning"
                  ...
                  &bull; First name too long, max. {{$v.firstName.$params.maxLength.max}} letters.
                </p>
              </div>

              <div class="input-group mb-3">
                <span class="input-group-text" id="input-website">* Your website</span>
                <input v-model="website"
                       type="url" class="form-control" placeholder="your-website.com" aria-label="Website" aria-describedby="input-website">
              </div>
              <div class="my-0">
                <p class="my-0 ps-2 text-start text-warning"
                   v-if="!$v.website.required">
                  &bull; Website url is required
                </p>
                <p class="my-0 ps-2 text-start text-warning"
                   v-if="!$v.website.url">
                  &bull; Enter valid url (ex. 'https://www.your-website.com')
                </p>
              </div>

              <div class="input-group mb-3">
                <span class="input-group-text" id="input-desc">* About you</span>
                <textarea v-model="description" class="form-control" rows="3" placeholder="Tell us little bit about you..."></textarea>
              </div>
              ...
              <div class="my-0">
                <p class="my-0 ps-2 text-start"
                   v-bind:class="!$v.password.minLength || !$v.password.required ? 'text-warning' : 'text-info fw-bold'">
                  &bull; Min. {{$v.password.$params.minLength.min}} characters
                  <span v-if="$v.password.minLength && $v.password.required">&check;</span>
                </p>
                <p class="my-0 ps-2 text-start"
...
<script>
import { required, minLength, maxLength, url, email, sameAs } from 'vuelidate/lib/validators'
  ...
  methods: {
    register() {
      ...
      if (this.$v.$invalid) {
        ...
        const request = new Request('http://localhost:8100/api/v1/artists')
        const requestInit = {
          method: 'POST',
          headers: headers,
          body: JSON.stringify({
        ...
        fetch(request, requestInit)
            ...
            .then(data => {
              ...
              this.$auth_artist.loginWithRedirect({
                redirect_uri: "http://localhost:8080/pro"
              });
            })
      }
    }
