...
public class ManagementApi {
    ...
    @Autowired
    public ManagementApi(ManagementApiToken managementApiToken) {
        this.token = managementApiToken;
    }
    ...
    public Map<String, String> createNewArtist(String email, String password) {
        BodyObj bodyObj = new BodyObj(email, password, auth0_connection);
        ...
        if (response.isSuccess()) {
            map.put("artist_id", response.getBody().getObject().getString("user_id"));
        } else {
    ...
    public int assignRolesToUser(String userId, String roleName) {
        ...
        node.putArray("roles").add(roleId); // vidjet kako se doda array ?
